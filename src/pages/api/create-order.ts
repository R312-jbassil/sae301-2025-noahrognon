...
async function createLunette(pb: any, options: any, svg: string) {
	...
	return pb.collection('lunette').create({
		code_svg: svg,
...
export const POST: APIRoute = async ({ request }) => {
	...
	try {
		...
		const lunette = await createLunette(pb, options, svg);
		...
		const commande = await pb.collection('Commande').create({
			IdUtilisateur: pb.authStore.model.id,
			IdLunette: lunette.id,
			total: options.price
		});
